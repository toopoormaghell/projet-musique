...
QMap<int, QStringList> BDDMp3::RecupererMP3s(int Type)
{
    ...
    if ( Type == 25 )
    {
        ...
        queryStri = "Select Id_MP3, Chemin FROM MP3 M, Relations R, Album B WHERE R.Id_Relation = M.Id_Relation AND R.Id_Album = B.Id_Album";
    }
...
void BDDMp3::supprimerenBDD() const
{
    if ( id() != 0 & id() !=-1 )
    {
        ...
        queryStr="DELETE FROM RelPlaylist WHERE Id_Relation='" + QString::number( m_relation->id() ) + "'";
        madatabase.exec( queryStr );
    }
